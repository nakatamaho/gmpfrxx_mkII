...
COMMAND_RE = re.compile(r"^COMMAND\s+Raxpy\s+(\S+)\s+(\S+)\s+(.+)$")
...
def display_name(raw):
    return raw.removeprefix("Raxpy_mpfr_")
...
def plot_rows(rows, title, output):
    ...
    for x, avg, low, high in zip(x_positions, averages, minimums, maximums):
        ...
        ax.text(x, avg + label_pad, f"{avg:.1f}", va="bottom", ha="center", fontsize=9, fontweight="bold", color="#111111", rotation=90)
...
def main():
    ...
    parser.add_argument("--title-prefix", default="MPFR Raxpy repeat benchmark")
    ...
    plot_rows(serial, f"{args.title_prefix}: serial avg with min/max range", args.output_dir / f"{args.output_prefix}_serial.png")
    plot_rows(openmp, f"{args.title_prefix}: OpenMP avg with min/max range", args.output_dir / f"{args.output_prefix}_openmp.png")
