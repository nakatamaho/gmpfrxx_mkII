...
### `mpf_class` lifetime and precision

- `mpf_class` must own exactly one initialized `mpf_t`.
- Every initialized `mpf_t` must be cleared exactly once.
- Default construction must use the project-owned thread-local default precision, initially 512 bits.
- Prefer `mpf_init2` over `mpf_init` for wrapper-owned objects.
- Do not use GMP's process-global `mpf_set_default_prec()` as the project default policy.
- Existing-object assignment preserves destination precision.
- New expression materialization uses expression-derived precision.
- `mpf_class(42)` means numeric value 42 at the current project default precision, not precision construction.

### `mpfc_class` lifetime and precision

- `mpfc_class` is a GMP-only complex type built from two `mpf_class` components.
- Default construction should initialize both components at the current project `mpf_class` default precision, initially 512 bits.
- `mpfc_class` must not use MPFR or MPC.
