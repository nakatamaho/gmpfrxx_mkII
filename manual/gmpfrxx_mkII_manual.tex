...
\subsubsection*{\texttt{mpfr\_class} (MPFR real)}

\begin{lstlisting}
mpfrxx::mpfr_class a(1.5);                      // double, default precision
mpfrxx::mpfr_class b(1.5, 256);                 // double, 256 bits
mpfrxx::mpfr_class c(int{42});                  // any non-bool integral
mpfrxx::mpfr_class d(42L, 256);                 // integral + precision
mpfrxx::mpfr_class e(some_mpz);                 // mpz_class, default prec
mpfrxx::mpfr_class f(some_mpz, 256);            // mpz_class + precision
mpfrxx::mpfr_class g(some_mpq, 256);            // mpq_class + precision
mpfrxx::mpfr_class h(other_mpfr, 256);          // requantize precision
mpfrxx::mpfr_class i("3.14159");                // string at default prec
mpfrxx::mpfr_class j("3.14159", 256);           // string + precision
mpfrxx::mpfr_class k("3.14159", 256, 10);       // string + precision + base
mpfrxx::mpfr_class l(some_mpfr_srcptr);         // explicit, raw mpfr_t
mpfrxx::mpfr_class m(some_mpfr_srcptr, 256);    // raw mpfr_t + precision
\end{lstlisting}

\paragraph{Constructor diagnostics.} String constructors throw
\code{std::invalid\_argument} on parse failure. \code{bool} construction
and \code{bool} assignment are \code{= delete}d. Unlike \code{mpz\_class},
\code{mpfr\_class} does \emph{not} accept \code{\_\_int128\_t} or
\code{\_\_uint128\_t} as a constructor argument.
...
// MPFR (only with_precision(prec) and with_precision(prec, double))
auto r0 = mpfrxx::mpfr_class::with_precision(256);
auto r1 = mpfrxx::mpfr_class::with_precision(256, /*value=*/1.5);
...
\subsubsection*{\texttt{mpfr\_class} accessors and setters}

\begin{lstlisting}
mpfr_class r = mpfr_class::with_precision(256);
r.set(2.5);
r.set(int{42});                                    // any non-bool integral
r.set("3.14159");                                  // throws on failure
int rc = r.set_str("3.14159", 10);                 // non-throwing
int rc2 = r.set_str("0xff.0p-4", 0);               // base 0 = auto-detect

double      d = r.to_double();   // also get_d()
unsigned long u = r.get_ui();
signed long   v = r.get_si();
mpfr_prec_t   p = r.precision(); // also get_prec()

r.set_prec(512);          // round-aware precision change

bool ok = r.fits_slong_p();   // fits_sint_p, fits_sshort_p,
                              // fits_uint_p, fits_ulong_p, fits_ushort_p

// Explicit conversion to bool (true iff non-zero), mpz_class, mpq_class.
bool nz   = static_cast<bool>(r);
mpz_class as_z(static_cast<mpz_class>(r));    // truncates
mpq_class as_q(static_cast<mpq_class>(r));    // canonicalized

std::string s = r.to_string(/*n_digits=*/0);
std::string g = r.get_str();
mpfr_exp_t  e;
std::string g2 = r.get_str(e, /*base=*/10, /*n_digits=*/0);

// Static accessors mirroring the namespace-level defaults.
mpfr_prec_t dp = mpfr_class::default_precision();   // == default_precision_bits()
mpfr_rnd_t  dr = mpfr_class::default_rounding();    // == default_rounding_mode()
\end{lstlisting}

\noindent
\code{mpfr\_class} has only \code{set\_prec()} (round-aware, via
\code{mpfr\_prec\_round}); there is no \code{set\_prec\_raw} on this
class.
...
\code{mpfrxx::mpfr\_class} has \code{std::common\_type} specializations
for the three pairings \code{(mpfr\_class, mpz\_class)},
\code{(mpfr\_class, mpq\_class)}, and \code{(mpfr\_class, mpfr\_class)},
all yielding \code{mpfr\_class}. There are no \code{std::common\_type}
specializations between \code{mpfr\_class} and builtin scalar types,
nor for \code{mpc\_class}.
...
\code{mpf\_class} and \code{mpfr\_class} additionally support
power-of-two scaling via \code{operator<<=} and \code{operator>>=}
(shift by \code{unsigned long}), which delegate to
\code{mpf\_mul\_2exp}/\code{mpf\_div\_2exp} or
\code{mpfr\_mul\_2ui}/\code{mpfr\_div\_2ui} respectively. Pre- and
post-increment / decrement (\code{++}, \code{--}) are also defined for
\code{mpfr\_class} in both lvalue and rvalue forms.
...
Header: \code{<mpfrxx\_mkII.h>}. All wrappers delegate to MPFR (verified
against MPFR 4.x) at the materialized operand precision. The rounding
mode used is \code{mpfr\_class::default\_rounding()}, i.e.\ the
wrapper-owned default. Each function accepts both \code{mpfr\_class}
objects and expression nodes whose \code{result\_type} is
\code{mpfr\_class}, \code{mpz\_class}, or \code{mpq\_class};
exact-typed expressions are first evaluated in the GMP family and then
converted once via \code{mpfr\_set\_z} or \code{mpfr\_set\_q}.

\subsubsection*{Algebraic and ternary}
\code{sqrt}, \code{sqr}, \code{sqrt\_ui} (default- and
explicit-precision overloads), \code{rec\_sqrt}, \code{cbrt},
\code{rootn\_ui}, \code{rootn\_si}, \code{agm}, \code{neg};\\
\code{fma(a, b, c)}, \code{fms(a, b, c)};\\
\code{fmma(a, b, c, d)}, \code{fmms(a, b, c, d)};\\
\code{hypot}, \code{min}, \code{max}, \code{dim}.
...
\subsubsection*{Power}
\begin{sloppypar}
\code{pow(lhs, rhs)}, \code{powr(lhs, rhs)};
\code{pow\_si}, \code{pow\_ui}, \code{pow\_z}, \code{pow\_sj}
(\code{intmax\_t} exponent), \code{pow\_uj} (\code{uintmax\_t}
exponent);
\code{ui\_pow(base, expr)}, \code{ui\_pow\_ui(base, expr)};
\code{compound\_si(expr, exponent)}.
\end{sloppypar}
...
\code{sin}, \code{cos}, \code{tan};
\code{asin}, \code{acos}, \code{atan},
\code{atan2(y, x)}, \code{atan2pi(y, x)},
\code{atan2u(y, x, unit)};
\code{sec}, \code{csc}, \code{cot};
\code{sinu(expr, unit)}, \code{cosu(expr, unit)}, \code{tanu(expr, unit)},
\code{asinu(expr, unit)}, \code{acosu(expr, unit)},
\code{atanu(expr, unit)};
\code{sinpi}, \code{cospi}, \code{tanpi}, \code{asinpi},
\code{acospi}, \code{atanpi}.
\end{sloppypar}
...
\subsubsection*{Constants (default- and explicit-precision overloads)}
\code{const\_pi}, \code{const\_e}, \code{const\_log2}, \code{const\_log10},
\code{pi\_over\_two}, \code{const\_euler}, \code{const\_catalan}.

\subsubsection*{Sign and next-representable}
\begin{sloppypar}
\code{abs}, \code{signbit}, \code{sgn}, \code{setsign(expr, sign)},
\code{copysign(value, sign\_source)};
\code{nextabove}, \code{nextbelow},
\code{nexttoward(expr, direction)}, \code{nextafter(expr, direction)}.
\end{sloppypar}
...
\begin{sloppypar}
\code{cmp(lhs, rhs)} (returns \code{int}, sign of \code{lhs - rhs});
\code{cmpabs(lhs, rhs)}, \code{cmpabs\_ui(expr, ui)}, \code{reldiff},
\code{eq(lhs, rhs, bits)};
...
\begin{sloppypar}
\code{sin\_cos(expr)} returns \code{std::pair<mpfr\_class, mpfr\_class>};
\code{sinh\_cosh(expr)} returns \code{std::pair<mpfr\_class, mpfr\_class>};
\code{lgamma(expr)} returns \code{std::pair<mpfr\_class, int>} (value
plus sign);
\code{remquo(lhs, rhs)} returns \code{std::pair<mpfr\_class, long>}
(remainder plus quotient bits).
\end{sloppypar}

\subsubsection*{Modular and remainder}
\code{remainder(lhs, rhs)}, \code{fmod(lhs, rhs)}.

\subsubsection*{Special functions}
\begin{sloppypar}
\code{erf}, \code{erfc}, \code{gamma}, \code{reciprocal\_gamma},
\code{lngamma}, \code{digamma}, \code{zeta},
\code{j0}, \code{j1}, \code{y0}, \code{y1}, \code{ai};
\code{zeta\_ui}, \code{fac\_ui} (each with default- and
explicit-precision overloads);
\code{jn(order, expr)}, \code{yn(order, expr)},
\code{gamma\_inc(lhs, rhs)}, \code{beta(lhs, rhs)}.
\end{sloppypar}

\subsubsection*{Comparison operators}
The full set of comparison operators \code{==}, \code{!=}, \code{<},
\code{<=}, \code{>}, \code{>=} is defined for \code{mpfr\_class} against
any combination of \code{mpfr\_class} object, expression, or supported
scalar (in either operand position). The implementation uses the named
\code{*\_p} predicates above; in particular \code{a < b} is
\code{mpfr\_less\_p(a, b)}, which returns \code{false} if either
operand is NaN. Equality on NaN is \code{false} (consistent with IEEE).
...
\item \textbf{Standard-library traits are partial.}
  ...
  in the standard-library interoperability section). For
  ...
  for cross-class promotions; on the MPFR/MPC side, only the three
  \code{mpfr\_class}/\code{mp\{z,q,fr\}\_class} pairings are
  specialized\,---\,scalar pairings are not.
